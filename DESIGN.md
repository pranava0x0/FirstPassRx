...
- **Frame vocabulary forward, not deficit.** Load-bearing product terms set the user's mental model — *top need* and *upgrade opportunity* point forward where *weakness* points back. Avoid hostile or overly negative clinical terms like "Rejects" for patient-facing search/status badges; instead, use helpful, action-oriented labels like "Needs PA" or "Non-Preferred". Choose the framing in the project `design.md`, mirror it in field names and labels, and enforce it with a test that greps the build for banned words.
...
- **One surface, two audiences — lead lay, demote clinical.** When a tool serves both a layperson and an expert (a patient *and* a prescriber, a citizen *and* an analyst), lead with the plain-language answer and a concrete *what-to-do-next* action ladder; demote exact/clinical precision (codes, source-verification state) behind a native `<details>` the expert opens, not a mode toggle. Label the primary control in lay terms with the clinical token as a quiet secondary line ("Daily combo controller" over "ICS/LABA", not instead of it). Plain-language is the default view; the rigor is one tap away, never gone. But don't bury the *one* load-bearing number the expert needs every time (a dose, a strength) so deep that every visit costs a click — keep it in the open answer.
...
- **Section rhythm is owned by `.doc > *`, not by individual sections.** Every direct child of `.doc` (the results document — highlights, recommendation hero, alternatives stack, coverage detail, etc.) automatically gets a `border-top` divider + 16px top / 4px bottom padding from one shared rule, producing a consistent ~20px gap between every section. A section should NOT also add its own top/bottom margin for inter-section spacing — that stacks on top of the shared rhythm and reads as an inconsistent, oversized gap next to its siblings (found and fixed 2026-07-21: `.recommendation-hero` had a leftover `margin: 12px 0 var(--section-spacing)` predating the `.doc > *` system, nearly doubling the gap before "Alternatives" vs. every other section transition in the same document).
