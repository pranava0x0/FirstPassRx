...
- **Research agents (as opposed to data-gathering agents) should checkpoint their notes into a COMMITTED `docs/` path, not a gitignored scratch dir.** The 2026-08-24 PA-drafting landscape agents wrote incrementally to `docs/pa-drafting/notes/*.md` — files that git preserves past the worktree's lifetime — which removes the entire class of "checkpoint referenced by the backlog no longer exists" failures documented above for `data-gathering/<stamp>/`. Gathered *data* still goes through the merge-into-`formulary.json` pipeline; research *prose with citations* should be born committed.
...
- **Bulk-archiving third-party content (scraped pages, downloaded documents) can carry live credentials that don't belong to you or the user — scan contents, not just reachability/size, before committing.** A session that switched a formulary-PDF archive from gitignored to committed staged 214 third-party pages verbatim; one embedded a live Mapbox API key in a map widget's HTML, caught by GitHub's push protection only after the commit was already made (never pushed — see the repo's `issues.md` for the full incident). The mental-model gap: "these are formulary documents" (the data being extracted) doesn't generalize to "these are arbitrary web pages that can embed anything a map/analytics/translation widget needs" (everything else riding along in the same bytes). A one-time manual grep isn't a structural fix either — re-scanning with an actual pattern-matching pass (not eyeballing a 500MB diff) found 3 more live keys the first grep missed. If a task involves archiving fetched web content verbatim, build an automated secret scan into the fetch/save path itself (see `scripts/archive-sources.mjs`'s `redactSecrets()`), not a step to remember before each commit.
