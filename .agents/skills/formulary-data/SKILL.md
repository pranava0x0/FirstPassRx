...
- **Location** — the US state. Determines the Medicaid plan (e.g. MA → MassHealth) and which
  commercial payers/PBMs are in scope.
- **Payer(s) / insurance** — one or more plans. Each maps to a `meta.payers[]` entry with its
  `formularyUrl`, `paPolicyUrl`, and `pbm`.
- **Drug class / type** — one of the `meta.classes[]` ids (`saba`, `ics`, `icslaba`, `lama`,
  `biologics`) or a new class to add. A class can span asthma and/or COPD — record the indication.
...
- **Gov:** `*.gov` — `mass.gov`, `cms.gov`, `fda.gov` (Orange Book / Purple Book / first-generic
  lists), `nih.gov`, `ncbi.nlm.nih.gov`, `nhlbi.nih.gov`, `dailymed.nlm.nih.gov` (FDA labels).
- **Guideline:** `ginasthma.org`, GOLD report.
- **Payer:** the plan's own domain or its formulary host (e.g. `optumrx.com` content hub,
  `formularynavigator.com`, the Conduent MHDL host for MassHealth).
...
4. **Merge inline.** Read the checkpoint files (not just the workflow return value — they survive a
   dropped run), map each into a `FormularyRecord` with the right `verification` state, and update
   `src/data/formulary.json` + `meta.references`.
...
- Set `meta.dataStatus` (`verified` only when every cell is), `capturedAt`, and bump `version`.
