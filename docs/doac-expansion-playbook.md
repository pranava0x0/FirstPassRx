...
| --- | --- | --- | --- |
| PA | `pa-ace` | 3 (pa-medicaid, ibx-commercial, highmark-bcbs) | `pa-doac` |
...
`al-doac` (already shipped) and `ny-doac` are done. Order is not load-bearing — the user asked for
NY first, then "the rest"; no ordering was specified among PA/CA/MA/MD/VA/IL.
